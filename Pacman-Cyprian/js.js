//Seleccionem l'etiqueta  del html
...
//variable per comprovar el mapa 
var comprovar = false;
//
var boundaries = [];
// variable per establir que per defecte les tecles estan premudes
const keys = {
	w: { pressed: false },
	s: { pressed: false },
	d: { pressed: false },
...
class player {
	...
	draw() {
		c.save();
		c.translate(this.position.x, this.position.y); //mou el context del canvas al jugador
		...
		c.lineTo(this.position.x, this.position.y) //dibuixa  una linia des del final del arc fins el centre
		c.fillStyle = 'yellow';
		c.fill();
		c.closePath();
		c.restore();
...
//quan el jugador perdi una vida tornara a la posició inicial al igual que els fantasmes
function tornarAnimacio() {
	...
	setTimeout(function(){fantasmes.velocity.x=1;
		fantasmes.velocity.y=1;},2000)
...
function animation() {
	...
	if (keys.w.pressed && lastkey == 'w') {
		for (let i = 0; i < boundaries.length; i++) {
			...
			//comprovem si colisiona cap amunt
			if (
				colision({
					circle: {
						...Player,
